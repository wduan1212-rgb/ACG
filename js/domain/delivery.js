...
import { state, save, notify, accountById, assetById, canReview } from "../core/store.js";
...
/* 交付：生成交付资产记录，推进 production 到 delivered（仅审核权限可入供应商端） */
export function deliver(p) {
  const acc = accountById(p.accountId);
  if (!acc) return null;
  if (!canReview()) { window.__toast && window.__toast("只有审核员/管理员可交付入供应商端"); return null; }
