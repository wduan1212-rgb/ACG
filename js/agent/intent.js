...
export function parseGoalFallback(goal) {
  ...
  // 数量单位只认"个/条/只/家"（避免"做一期""三步"等被误当数量）
  const cnM = goal.match(/([0-9]+|[两一二三四五六七八九十]+)\s*(个|条|只|家)/);
  const count = cnM ? zhNum(cnM[1]) : null;
  ...
  return { topic: topic.slice(0, 30), tags, group, style: styleM ? styleM[1] : "", count };
}
