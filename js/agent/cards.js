...
const CARD = {
  ...
  plan(m) {
    ...
    return `<div class="ag-card plan ${confirmed ? "resolved" : ""}" data-plan="${m.id}">
      ...
      <div class="agc-grid">
        ...
        <label class="agc-field">风格策略<input data-pf="style" value="${esc(p.style || "")}" placeholder="默认用各账号自带创作风格" ${confirmed || cancelled ? "disabled" : ""} /></label>
      </div>
      ...
      ${matched.some(a => a.subType === "无数字人") ? (() => {
        const ref = p.sharedRefAssetId ? state.assets.find(x => x.id === p.sharedRefAssetId) : null;
        const editable = !confirmed && !cancelled;
        return `<div class="agc-ref ${editable ? "droppable" : ""}" ${editable ? `data-plan-refdrop="${m.id}"` : ""}>
          <span class="agc-ref-l">${icon("star", 12)} 统一参考图<em>素材号所有分镜共用（logo / 产品界面）· 可拖入</em></span>
          ${ref ? `<span class="ref-chip dark">${urlFor(ref.id) ? `<img src="${urlFor(ref.id)}"/>` : ""}<span>${esc(ref.name)}</span>${editable ? `<button class="ref-x" data-act="plan-refclear" data-mid="${m.id}">${icon("x", 10)}</button>` : ""}</span>`
            : !editable ? `<span class="muted">未设置</span>` : `<label class="btn ghost sm">${icon("upload", 12)} 上传<input type="file" accept="image/*" hidden data-plan-ref="${m.id}" /></label>`}
        </div>`;
      })() : ""}
  ...
  progress(m) {
    ...
    prods.forEach(p => {
      ...
      else if (p.stage === "render" || p.stage === "workshop") c.gen++;
...
export function boardRow(p) {
  ...
  // 渲染中的细进度
  let sub = "";
  if ((p.stage === "render" || p.stage === "workshop") && p.stageStatus === "running") {
    const jobs = jobsOf(p);
