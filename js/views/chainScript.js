    ...
    <div class="chain-page">
      <div class="chain-main">
        <div class="page-head">
          ...
          <h2>${isImg ? "AI 依账号定位生成小红书笔记图卡" : material ? "AI 生成长视频口播脚本（60s+ · 有深度/有梗）" : "AI 依账号定位生成分镜脚本（含口播）"}</h2></div>
          <button class="btn primary" id="csNext">下一步：${isImg ? "成图" : material ? "分镜工坊" : "分镜"} ${icon("arrowRight", 14)}</button>
        </div>
  ...
  $("#csGen", root).addEventListener("click", e => withLoading(e.currentTarget, async () => {
    ...
    const res = material
      ...
      : await AI.generateScript({
        ...
        style, imageCount: A.imageCount || 6
      });
    A.shots = res.shots || [];
    A.title = res.title || topic;
    A.source = AI.lastSource;
    p.title = res.title || topic;
    if (!isImg && !material) normalizeVideoTimes(A.shots);
    if (material) { Object.assign(p.artifacts.audio, { assetId: null, duration: 0, perShot: [], source: "" }); }
