...
function defaultTab(p) {
  ...
  if (p.stage === "render" || p.stage === "cut" || p.stage === "workshop") return "render";
...
export function stagePage(p) {
  ...
  if (p.mode === "视频" && p.subType === "无数字人" && ["boards", "prompts", "render"].includes(p.stage)) return "workshop";
...
const TAB = {
  ...
  boards(p) {
    if (p.subType === "无数字人" && p.mode === "视频") {
      ...
      return `<div class="pd-note">${units.length} 个分镜单元 · 含界面/logo/中文走图生视频，其余文生视频</div>
