...
function computeStats() {
  ...
  return {
    日期: { 今天: today, 昨天: yest },
    账号: state.accounts.map(a => ({ 名称: a.name, 平台: a.platform, 分组: groupOf(a), 本月交付: a.monthlyDone || 0 })),
    任务阶段分布: stageCount,
...
async function askData(q) {
  ...
  try {
    const r = await AI.chat([
      { role: "system", content: `你是 ACG TV 内容工作台的数据助理，只负责"读数据回答问题"，绝不执行任何操作。只能依据下面这份 JSON 数据回答，数字必须与数据一致，数据里没有的就直说没有。回答用简洁中文，最多 3 行，不用 markdown。\n数据：${JSON.stringify(stats)}` },
      { role: "user", content: q }
    ]);
    ...
    root.innerHTML = `
      <div class="overview">
        <div class="ov-hero card">
          <div class="ovh-left">
            <div class="eyebrow">ACG TV · 内容生产工作台</div>
          ...
          <div class="ov-acc-grid ov-scroll">
            ${state.accounts.map(a => `
              <button class="ov-acc" data-acc="${a.id}" title="${esc(a.name)}">
                ...
                <span class="ovt-main"><b>${esc(a.name)}</b><em>${groupOf(a)} · ${a.monthlyDone || 0} 条</em></span>
