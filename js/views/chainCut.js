...
export function renderCutPage(root, p) {
      ...
      <div class="cut-top">
        ...
        <aside class="cut-side">
          <div class="side-card card">
            <h3>${icon("wand", 14)} 智能${isMaterial(p) ? "混剪" : "剪辑"}</h3>
            <p class="muted">${isMaterial(p) ? "按口播时长拼片段 + 铺字幕 + 自动选配 BGM，再来精修。" : "自动按片段顺序拼接 + 从口播铺字幕，再来精修。"}</p>
            <button class="btn gen block" id="cutAuto">${icon("spark", 14)} 一键智能${isMaterial(p) ? "混剪" : "拼接"}</button>
          </div>
          ${isMaterial(p) ? `
          <div class="side-card card">
  ...
  $("#cutAuto", root).addEventListener("click", () => {
    ...
    if (!r.clips && !TL().length) { toast(isMaterial(p) ? "还没有就绪片段：回分镜工坊「一键全自动」生成" : "还没有可用片段：先去生成台生成，或等 Agent 渲染完成"); return; }
    renderCutPage(root, p);
    toast(`智能${isMaterial(p) ? "混剪" : "拼接"}完成：${TL().length} 段 + ${SUBS().length} 条字幕${r.bgm ? ` · BGM「${r.bgm}」` : ""}`);
  });
  ...
  if (bgmSel) bgmSel.addEventListener("change", e => {
    ...
    save("productions");
    toast(name ? `BGM 已换为「${name}」` : "已移除 BGM");
  });
